...
export function generateAlternatingPulseMap(
  config: BasicPulseConfig
): { brightness: number; direction: 'positive' | 'negative' | 'both' }[] {
...
export function generatePulseScene(config: { speed: number; brightness: number }, interval: number): Scene {
  const totalDuration = 5000 - Math.floor(4800 * (config.speed / 100));
  const pulseMap = generateBasicPulseMap({
    minBrightness: 5,
    maxBrightness: config.brightness,
    totalDuration,
    interval,
